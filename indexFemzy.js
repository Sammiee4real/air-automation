...
async function getDataBalance(txnId,balanceType, page){
    ...
    console.log("Getting data balance")
    await page.goto('https://selfcare.airtel.com.ng/EducationSuite/Contract/Balance',{ timeout: 0, waitUntil: 'networkidle0' }).catch(err=>console.log('err', err.message));
    // sleep(10000)
    // await page.waitForNavigation(0);  

    //shows the page auth failed, no cookie is availables
    // if (getCookie('edusuite') === null) {
      // console.log(await page.$('#tblpaymentHistory_filter'))
    if (await page.$('#tblpaymentHistory_filter') == null) {
      ...
      // console.log('balance was logged');l

      fetch(`${baseApiUrl}fetch-and-save-balance`, {
        method: 'POST',
        headers: {
...
async function vendingFunction(page){
  // const sleep = (millsec) => {
  //   return new Promise( (resolve) => setTimeout(resolve,millsec));
  // } 
       ...
       for(const eachTxn of response.data){   
        ...
        if(dataSize == 'nil'){
          ...
          try{
              ...
              await getDataBalance(txnId,'before', page).catch('Get Balance Before Error:  ');
              ...
              if(eachTxn.status == 0){
                ...
                //  dataSize
                let responsePup =  await main(phone,dataSize,txnId, page).catch(console.error);
                ...
                if(resultt.status == 'success'){
                  //call an api here
                    ...
                    .then( async (response) => {
                      // console.log(response);
                      await getDataBalance(txnId,'after', page).catch('Get Balance After Error:  ');
                    });      
...
async function main(phone,dataSize,txn_id, page) {

  console.log('Phone Vending Checker:::: ',checker);

  if (checker.includes(txn_id)){
    return { 'status' : 'failed','message' : 'Sorry seems this transaction ID: '+txn_id+' was vended a while ago' };
  }
...
async function resetCookie(page){
...
async function performTasks(page) {
  try {
    if (processing) {
      // If a loop is already in progress, wait and retry
      await sleep(1000); // Adjust the delay time as needed
      return await performTasks(page);
    }
    ...
    console.log("Started");
    await vendingFunction(page);
    // await getDataBalance(page);

    // Introduce a delay before the next iteration if needed
    await sleep(5000); // Adjust the delay time as needed

    processing = false;

    // Recursively call performTasks
    ...
    // Introduce a delay before the next recursive call in case of an error
    await sleep(5000); // Adjust the delay time as needed

    // Recursively call performTasks after handling the error
    await performTasks(page);
  }
}
...
async function initialize() {
  ...
  // Start the recursion by calling performTasks with the page instance
  await performTasks(page)
    .then(() => console.log("Task execution started"))
    .catch(err => console.log("Error starting execution: ", err));
